...
317 tests: 29 in `shared` (protocol and REST schemas), 111 in `backend` (hub, backpressure, limits, upstream parsing, freshness, routes, and a Fastify + `ws` integration suite), 177 in `mobile` (stream client state machine, store and render isolation, screens, order book, storage, telemetry).
